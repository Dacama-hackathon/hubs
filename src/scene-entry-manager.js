...
export default class SceneEntryManager {
  ...
  _setupPlayerRig = () => {
    ...
    if (avatarScale) {
      this.avatarRig.setAttribute("scale", {
        x: avatarScale,
        y: avatarScale,
        z: avatarScale
      });
  ...
  _setupKicking = () => {
    ...
    document.body.addEventListener("kicked", ({
      detail
    }) => {
      const {
  ...
  _pinElement = async el => {
    const {
      networkId
    } = el.components.networked.data;

    const {
      fileId,
      src
    } = el.components["media-loader"].data;
  ...
  _setupMedia = mediaStream => {
    this.scene.addEventListener("add_media", e => {
      ...
      console.log(e.detail, contentOrigin)
      this.spawnMediaInfrontOfPlayer(e.detail, contentOrigin);
    });
    ...
    this.scene.addEventListener("action_mute_client", ({
      detail: {
        clientId
      }
    }) => {
  ...
  _setupCamera = () => {
    ...
    this.scene.addEventListener("photo_taken", e => this.hubChannel.sendMessage({
      src: e.detail
    }, "photo"));
    this.scene.addEventListener("video_taken", e => this.hubChannel.sendMessage({
      src: e.detail
    }, "video"));
  };
