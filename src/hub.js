...
store.update({
  preferences: {
    shouldPromptForRefresh: undefined
  }
}); // Clear flag that prompts for refresh from preference screen
...
async function updateEnvironmentForHub(hub, entryManager) {
  ...
  const sceneErrorHandler = () => {
    remountUI({
      roomUnavailableReason: "scene_error"
    });
  ...
  const connectToScene = async () => {
      ...
      const sendViaPhoenix = reliable => (clientId, dataType, data) => {
        ...
        if (isOpen || reliable) {
          ...
          if (hasFirstSync) {
            ...
            hubChannel.channel.push("nafr", {
              naf: JSON.stringify(nafrPayload)
            });
          }
        }
      };
    ...
    const loadEnvironmentAndConnect = () => {
      ...
      function onConnectionError() {
        ...
        remountUI({
          roomUnavailableReason: "connect_error"
        });
        entryManager.exitScene();
      }
      ...
      scene.components["networked-scene"]
        ...
        .catch(connectError => {
          ...
          remountUI({
            roomUnavailableReason: isFull ? "full" : "connect_error"
          });
          entryManager.exitScene();
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    performConditionalSignIn(
      ...
      async () => {
        ...
        if (hubChannel.can("tweet")) {
          ...
          isInOAuth = true;
          store.enqueueOnLoadAction("emit_scene_event", {
  ...
  if (qs.get("required_version") && process.env.BUILD_VERSION) {
    ...
    if (qs.get("required_version") !== buildNumber) {
      remountUI({
        roomUnavailableReason: "version_mismatch"
      });
  ...
  socket.onClose(e => {
    ...
    if (e.code === NORMAL_CLOSURE && !isReloading) {
      entryManager.exitScene();
      remountUI({
        roomUnavailableReason: "disconnected"
      });
    }
  });
